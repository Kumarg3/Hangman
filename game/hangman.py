...
class hangman:
    ...
    def __init__(self, wrong_choice_counter=5,winner_point=10):
        '''Initialise parameter in constructor for each game
            player_guess_counter: number of choices that can be wrong during one word guess, It will be used to reset counter at each new puzzle
    ...
    def check_choice(self,player_guess_word):
        ...
        if len(player_guess_word) != 1:
            ...
            if self.in_play == 0:
                ...
                message = 'Well done, You are a star'
            else:
                message = 'Keep Going'
        else:
    ...
    def __get_screen_puzzle__(self):
        ...
        word = [self.puzzle_word[key] if value == True else '*' for (key,value) in self.puzzle_word_index.items()]
        return ''.join(word)
